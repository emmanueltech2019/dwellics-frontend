import React from "react";

function partner() {
  return (
    <div className="content-wrapper">
      <div
        className="wp-block-niche-group wp-block-niche-group--has-squiggle wp-block-niche-group--squiggle-middle"
        id="niche-group-f80f5c02-dbb4-4d71-aeed-90d287176148"
      >
        <div className="wp-block-niche-group__inner">
          <div
            className="wp-block-niche-group niche-blocks--max-width-700"
            id="niche-group-91e44f47-10c0-4da2-ac3d-23cabe237db4"
            style={{ padding: "80px 20px 50px 20px" }}
          >
            <div className="wp-block-niche-group__inner">
              <div
                id="text-block_5edfd8c6973b5"
                style={{ padding: "10px" }}
                className="Expedite-Text Expedite-Text__niche--600 Expedite-Text__weight--600 niche-blocks--leading-12 niche-blocks--color-green02 niche-blocks--text-align-left"
              >
                Partnerships
              </div>
              <div
                id="text-block_5edfd8aa973b4"
                style={{ paddingBottom: "20px" }}
                className="Expedite-Text Expedite-Text__source--800 Expedite-Text__weight--400 niche-blocks--leading-11 niche-blocks--color-gray02 niche-blocks--text-align-left"
              >
                Dwellics helps business and companies reach millions of prospective
                 homebuyers every year.
              </div>

              <div
                id="text-block_5edfd916973b6"
                className="Expedite-Text Expedite-Text__source--500 Expedite-Text__weight--400 niche-blocks--leading-14 niche-blocks--color-gray02 niche-blocks--text-align-left"
              >
                
Dwellics is the go-to partner for real estate and mortgage brokers/agents who desire to have access to a wide array of homebuyers and mortgage buyers from across the US annually.
              </div>
            </div>
          </div>

          <div
            className="wp-block-niche-group niche-blocks--max-width-820 wp-block-niche-group--card p-20 md:p-[50px]"
            id="niche-group-10573d2e-129d-4fd2-9a65-554e51d4c60f"
          >
            <div className="wp-block-niche-group__inner">
              <div className="is-layout-flex wp-container-4 wp-block-columns">
                <div className="is-layout-flow wp-block-column">
                  <div className="wp-block-image">
                    <figure className="aligncenter size-large is-resized">
                      <img
                        decoding="async"
                        src="https://www.niche.com/about/wp-content/uploads/2018/04/school-2.png"
                        alt=""
                        className="wp-image-2409"
                        width="125"
                        height="83"
                      />
                    </figure>
                  </div>

                  <div
                    id="text-block_5edfd988973b7"
                    style={{ paddingBottom: "10px" }}
                    className="Expedite-Text Expedite-Text__source--500 Expedite-Text__weight--600 niche-blocks--leading-12 niche-blocks--color-gray02 niche-blocks--text-align-left"
                  >
                   Real estate brokers/agents and mortgage brokers/agents Partnership
                  </div>

                  <div
                    id="text-block_5edfd9ad973b8"
                    style={{ paddingBottom: "10px" }}
                    className="Expedite-Text Expedite-Text__source--400 Expedite-Text__weight--400 niche-blocks--leading-14 niche-blocks--color-gray02 niche-blocks--text-align-left"
                  >
                    Partnership with Dwellics offers golden opportunities for real estate and mortgage brokers and agents to track their business profiles on Dwellics
Learn more
                  </div>

                  <div
                    id="text-block_5edfda82973b9"
                    style={{ paddingBottom: "28px" }}
                    className=""
                  >
                    <a
                      href="#schools"
                      className="Expedite-Button--button Expedite-Button__text--indigo02 Expedite-Button__bg--white"
                    >
                      <span className="Expedite-Text Expedite-Text__source--400 Expedite-Text__weight--600">
                        Learn More
                      </span>
                    </a>
                  </div>
                </div>

                <div className="is-layout-flow wp-block-column">
                  <div className="wp-block-image">
                    <figure className="aligncenter size-large is-resized">
                      <img
                        decoding="async"
                        loading="lazy"
                        src="https://www.niche.com/about/wp-content/uploads/2018/04/sign-new-1.png"
                        alt=""
                        className="wp-image-2406"
                        width="125"
                        height="83"
                      />
                    </figure>
                  </div>

                  <div
                    id="text-block_5edfdb1d973ba"
                    style={{ paddingBottom: "10px" }}
                    className="Expedite-Text Expedite-Text__source--500 Expedite-Text__weight--600 niche-blocks--leading-12 niche-blocks--color-gray02 niche-blocks--text-align-left"
                  >
                   Buy Dwellics Data
                  </div>

                  <div
                    id="text-block_5edfdb22973bb"
                    style={{ paddingBottom: "10px" }}
                    className="Expedite-Text Expedite-Text__source--400 Expedite-Text__weight--400 niche-blocks--leading-14 niche-blocks--color-gray02 niche-blocks--text-align-left"
                  >
                    Partnership with Dwellics offers golden opportunities for real estate and mortgage brokers and agents to track their business profiles on Dwellics
Learn more
                  </div>

                  <div
                    id="text-block_5edfe1ac0b859"
                    style={{ paddingBottom: "28px" }}
                    className=""
                  >
                    <a
                      href="#data"
                      className="Expedite-Button--button Expedite-Button__text--indigo02 Expedite-Button__bg--white"
                    >
                      <span className="Expedite-Text Expedite-Text__source--400 Expedite-Text__weight--600">
                        Learn More
                      </span>
                    </a>
                  </div>
                </div>

                <div className="is-layout-flow wp-block-column">
                  <div className="wp-block-image">
                    <figure className="aligncenter size-large is-resized">
                      <img
                        decoding="async"
                        loading="lazy"
                        src="https://www.niche.com/about/wp-content/uploads/2018/04/plane-2.png"
                        alt=""
                        className="wp-image-2410"
                        width="125"
                        height="83"
                      />
                    </figure>
                  </div>

                  <div
                    id="text-block_5edfdb6e973bc"
                    style={{ paddingBottom: "10px" }}
                    className="Expedite-Text Expedite-Text__source--500 Expedite-Text__weight--600 niche-blocks--leading-12 niche-blocks--color-gray02 niche-blocks--text-align-left"
                  >
                   Advertise on Dwellics
                  </div>

                  <div
                    id="text-block_5edfdb79973bd"
                    style={{ paddingBottom: "10px" }}
                    className="Expedite-Text Expedite-Text__source--400 Expedite-Text__weight--400 niche-blocks--leading-14 niche-blocks--color-gray02 niche-blocks--text-align-left"
                  >
                    Get connected to your target audience, homebuyers and loan buyers actively looking for real estate, mortgage and loan opportunities
                  </div>

                  <div
                    id="text-block_5edfe1b10b85a"
                    style={{ paddingBottom: "28px" }}
                    className=""
                  >
                    <a
                      href="#advertise"
                      className="Expedite-Button--button Expedite-Button__text--indigo02 Expedite-Button__bg--white"
                    >
                      <span className="Expedite-Text Expedite-Text__source--400 Expedite-Text__weight--600">
                        Learn More
                      </span>
                    </a>
                  </div>
                </div>
              </div>
            </div>
          </div>

          <div
            className="wp-block-niche-group niche-blocks--max-width-820"
            id="niche-group-50730f93-e0e1-4f11-b66e-184bca3055cb"
            style={{ padding: "80px 20px 30px 20px" }}
          >
            <div className="wp-block-niche-group__inner">
              <div className="is-layout-flex wp-container-8 wp-block-columns">
                <div className="is-layout-flow wp-block-column">
                  <div
                    id="text-block_5edfdc40973c0"
                    style={{ paddingBottom: "10px" }}
                    className="Expedite-Text Expedite-Text__niche--800 Expedite-Text__weight--700 niche-blocks--leading-11 niche-blocks--color-gray04 niche-blocks--text-align-center"
                  >
                    10,000+
                  </div>

                  <div
                    id="text-block_5edfdc64973c1"
                    style={{ paddingBottom: "30px" }}
                    className="Expedite-Text Expedite-Text__source--400 Expedite-Text__weight--400 niche-blocks--leading-12 niche-blocks--color-gray02 niche-blocks--text-align-center"
                  >
                    annual ratings and reviews from our users
                  </div>
                </div>

                <div className="is-layout-flow wp-block-column">
                  <div
                    id="text-block_5edfe2790b85b"
                    style={{ paddingBottom: "10px" }}
                    className="Expedite-Text Expedite-Text__niche--800 Expedite-Text__weight--700 niche-blocks--leading-11 niche-blocks--color-gray04 niche-blocks--text-align-center"
                  >
                    15,000
                  </div>

                  <div
                    id="text-block_5edfe2840b85c"
                    style={{ paddingBottom: "30px" }}
                    className="Expedite-Text Expedite-Text__source--400 Expedite-Text__weight--400 niche-blocks--leading-12 niche-blocks--color-gray02 niche-blocks--text-align-center"
                  >
                   comprehensive profiles on brokers, agents, companies and neighborhoods
                  </div>
                </div>

                <div className="is-layout-flow wp-block-column">
                  <div
                    id="text-block_5edfe2900b85d"
                    style={{ paddingBottom: "10px" }}
                    className="Expedite-Text Expedite-Text__niche--800 Expedite-Text__weight--700 niche-blocks--leading-11 niche-blocks--color-gray04 niche-blocks--text-align-center"
                  >
                    1,000,000+
                  </div>

                  <div
                    id="text-block_5edfe29d0b85e"
                    className="Expedite-Text Expedite-Text__source--400 Expedite-Text__weight--400 niche-blocks--leading-12 niche-blocks--color-gray02 niche-blocks--text-align-center"
                  >
                    yearly visitors use Dwellics
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="wp-block-niche-group__squiggle">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1547.32 7992.92">
            <path
              d="M0,52S58.06,21.5,125.27,31.81,232.68,60.2,297.56,51.62s87.91-25.06,137.6-19.07,75.41,27.06,125.66,20.73C620.91,45.72,646.68,19,698.64,13.14,763.76,5.86,800.78,47,873,42.4S953.73,9.48,1004.8,1,1115.61,40.33,1201,37.22c119.88-4.36,137.35-34.87,207.06-37s139.29,37.05,139.29,37.05v7955.7H0Z"
              fill="#e4f2e8"
            ></path>
          </svg>
        </div>
      </div>

      <div
        className="wp-block-niche-group niche-blocks--max-width-820 md:pt-[70px] pt-[40px] pr-[20px] pl-[20px] "
        id="niche-group-e326a273-e3d2-4d97-a7d1-a0af3546f6f2"
      >
        <div className="wp-block-niche-group__inner">
          <div className="is-layout-flex wp-container-15 wp-block-columns">
            <div
              className="is-layout-flow wp-block-column"
              style={{ flexBasis: "20%" }}
            >
              <div className="wp-block-image">
                <figure className="aligncenter size-large is-resized">
                  <img
                    decoding="async"
                    loading="lazy"
                    src="https://www.niche.com/about/wp-content/uploads/2018/04/partnerships.png"
                    alt=""
                    className="wp-image-2190"
                    width="125"
                    height="125"
                    srcset="
                      https://www.niche.com/about/wp-content/uploads/2018/04/partnerships.png         250w,
                      https://www.niche.com/about/wp-content/uploads/2018/04/partnerships-150x150.png 150w
                    "
                    sizes="(max-width: 125px) 100vw, 125px"
                  />
                </figure>
              </div>
            </div>

            <div
              className="is-layout-flow wp-block-column"
              style={{ flexBasis: "80%" }}
            >
              <div
                id="text-block_5edfe3070b85f"
                style={{ paddingBottom: "12px" }}
                className="Expedite-Text Expedite-Text__source--800 Expedite-Text__weight--400 niche-blocks--leading-12 niche-blocks--color-gray02 niche-blocks--text-align-left"
              >
                Real estate brokers/agents and mortgage brokers/agents Partnership
              </div>

              <div
                id="text-block_5edfe35e0b860"
                style={{ paddingBottom: "28px" }}
                className="Expedite-Text Expedite-Text__source--400 Expedite-Text__weight--400 niche-blocks--leading-14 niche-blocks--color-gray02 niche-blocks--text-align-left"
              >
              Dwellics gives you the opportunity to manage your business/company profile without paying a dime. All you need is to become a premium partner so more visitors can visit and interact with you.
              </div>

              <div className="is-layout-flex wp-container-13 wp-block-columns">
                <div className="is-layout-flow wp-block-column">
                  <div
                    id="text-block_5edfe4130b861"
                    style={{ paddingBottom: "12px" }}
                    className="Expedite-Text Expedite-Text__source--500 Expedite-Text__weight--600 niche-blocks--leading-12 niche-blocks--color-gray02 niche-blocks--text-align-left"
                  >
                    Claim your Data
                  </div>

                  <div
                    id="text-block_5edfe5885d850"
                    style={{ paddingBottom: "4px" }}
                    className="Expedite-Text Expedite-Text__source--400 Expedite-Text__weight--400 niche-blocks--leading-14 niche-blocks--color-gray02 niche-blocks--text-align-left"
                  >
                   Keep your profile fresh and track engagement to drive data
                  </div>

                  <div
                    id="text-block_5edfe44c0b863"
                    style={{ paddingBottom: "28px" }}
                    className=""
                  >
                    <a
                      href="#"
                      className="Expedite-Button--button Expedite-Button__text--indigo02"
                    >
                      <span className="Expedite-Text Expedite-Text__source--400 Expedite-Text__weight--600">
                        Claim Your School
                      </span>
                    </a>
                  </div>
                </div>

                <div className="is-layout-flow wp-block-column">
                  <div
                    id="text-block_5edfe8685d851"
                    style={{ paddingBottom: "12px" }}
                    className="Expedite-Text Expedite-Text__source--500 Expedite-Text__weight--600 niche-blocks--leading-12 niche-blocks--color-gray02 niche-blocks--text-align-left"
                  >
                    Go Premium
                  </div>

                  <div
                    id="text-block_5edfe8725d852"
                    style={{ paddingBottom: "4px" }}
                    className="Expedite-Text Expedite-Text__source--400 Expedite-Text__weight--400 niche-blocks--leading-14 niche-blocks--color-gray02 niche-blocks--text-align-left"
                  >
                   With our premium subscription, you can make your profile outstanding while enjoying the benefits of our market insights and solutions

                  </div>

                  <div
                    id="text-block_5edfe87e5d853"
                    style={{ paddingBottom: "28px" }}
                    className=""
                  >
                    <a
                      href="#"
                      className="Expedite-Button--button Expedite-Button__text--indigo02"
                    >
                      <span className="Expedite-Text Expedite-Text__source--400 Expedite-Text__weight--600">
                      Learn more about the Premium Partnership

                      </span>
                    </a>
                  </div>
                </div>

                <div className="is-layout-flow wp-block-column">
                  <div
                    id="text-block_5edfe8905d854"
                    style={{ paddingBottom: "12px" }}
                    className="Expedite-Text Expedite-Text__source--500 Expedite-Text__weight--600 niche-blocks--leading-12 niche-blocks--color-gray02 niche-blocks--text-align-left"
                  >
                    Promote your Brand
                  </div>

                  <div
                    id="text-block_5edfe89c5d855"
                    style={{ paddingBottom: "4px" }}
                    className="Expedite-Text Expedite-Text__source--400 Expedite-Text__weight--400 niche-blocks--leading-14 niche-blocks--color-gray02 niche-blocks--text-align-left"
                  >
                    You have access to promotional materials from Dwellics, including grades and rankings which you can exhibit on your site.
                  </div>

                  <div
                    id="text-block_5edfe8a75d856"
                    style={{ paddingBottom: "20px" }}
                    className=""
                  >
                    <a
                      href="#"
                      className="Expedite-Button--button Expedite-Button__text--indigo02"
                    >
                      <span className="Expedite-Text Expedite-Text__source--400 Expedite-Text__weight--600">
                        Learn more
                      </span>
                    </a>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>

      <div
        className="wp-block-niche-group py-[80px]"
        id="niche-group-0d8cb6b3-69ac-4218-bbeb-381574580cba"
        style={{backgroundColor:"#e4f2e8"}}
      >

        <div className="wp-block-niche-group__inner">
          <div
            className="wp-block-niche-group niche-blocks--max-width-900 wp-block-niche-group--card 
            pt-[40px] pl-[20px] pb-[20px] pr-[20px] 
            pt-[40px] pl-[40px] pb-[20px] md:pr-[40px]"
            id="niche-group-330b7e80-2936-4e7c-b495-11bcc0b1be6c"
          >
            <div className="wp-block-niche-group__inner">
              <div className="is-layout-flex wp-container-21 wp-block-columns">
                <div
                  className="is-layout-flow wp-block-column"
                  style={{ flexBasis: "20%" }}
                >
                  <div className="wp-block-image">
                    <figure className="aligncenter size-large is-resized">
                      <img
                        decoding="async"
                        loading="lazy"
                        src="https://www.niche.com/about/wp-content/uploads/2018/04/sign-new.png"
                        alt=""
                        className="wp-image-2200"
                        width="125"
                        height="125"
                        srcset="
                          https://www.niche.com/about/wp-content/uploads/2018/04/sign-new.png         250w,
                          https://www.niche.com/about/wp-content/uploads/2018/04/sign-new-150x150.png 150w
                        "
                        sizes="(max-width: 125px) 100vw, 125px"
                      />
                    </figure>
                  </div>
                </div>

                <div
                  className="is-layout-flow wp-block-column"
                  style={{ flexBasis: "80%" }}
                >

                  <div
                    id="text-block_5edfe9ae5d858"
                    className="Expedite-Text Expedite-Text__source--800 Expedite-Text__weight--400 niche-blocks--leading-12 niche-blocks--color-gray02 niche-blocks--text-align-left pb-[20px]"
                  >
                   Buy Dwellics Data

                  </div>
                  <div
                    id="text-block_5edfe9cc5d859"
                    className="Expedite-Text Expedite-Text__source--400 Expedite-Text__weight--400 niche-blocks--leading-14 niche-blocks--color-gray02 niche-blocks--text-align-left pb-[28px]"
                  >
                   At Dwellics, we boast of robust, all-inclusive data which immensely benefits real estate brokers and agents, mortgage brokers and agents, and moving companies. We provide authentic public data sets thoroughly analyzed in a way that makes it super useful and relevant to ourto you our client, helping you successfully attain business growth.

                  </div>

                  <div className="is-layout-flex wp-container-19 wp-block-columns">
                    <div className="is-layout-flow wp-block-column">

                      <div
                        id="text-block_5edfeba25d85b"
                        className="Expedite-Text pb-[12px] Expedite-Text__source--500 Expedite-Text__weight--600 niche-blocks--leading-12 niche-blocks--color-gray02 niche-blocks--text-align-left"
                      >
                       Market Insights
                      </div>

                      <div
                        id="text-block_5edfebb25d85c"
                        className="Expedite-Text pb-[28px] md:pb-[0px] Expedite-Text__source--400 Expedite-Text__weight--400 niche-blocks--leading-14 niche-blocks--color-gray02 niche-blocks--text-align-left"
                      >
                       Diverse Companies and startups find Dwellics data useful for marketing, lead generation, and growth. Our data comes from impeccable sources and are analyzed with our clients in mind.

                      </div>
                    </div>

                    <div className="is-layout-flow wp-block-column">

                      <div
                        id="text-block_5edfebfe5d85d"
                        className="Expedite-Text pb-[12px] Expedite-Text__source--500 Expedite-Text__weight--600 niche-blocks--leading-12 niche-blocks--color-gray02 niche-blocks--text-align-left"
                      >
                       Exhibit Our Data on your Website

                      </div>

                      <div
                        id="text-block_5edfec105d85e"
                        className="Expedite-Text pb-[28px] md:pb-[0px] Expedite-Text__source--400 Expedite-Text__weight--400 niche-blocks--leading-14 niche-blocks--color-gray02 niche-blocks--text-align-left"
                      >
                       You can include Dwellics neighborhood grades in your Web or social media real estate or mortgage content.

                      </div>
                    </div>
                  </div>

                  <div
                    id="text-block_5edfec4c5d85f"
                    className="niche-blocks--block-align-right"
                  >
                    <a
                      href="#"
                      className="Expedite-Button--button Expedite-Button__text--indigo02 Expedite-Button__padding--comfy"
                    >
                      <span className="Expedite-Text Expedite-Text__source--500 Expedite-Text__weight--600">
                        Learn More About Licensing Our Data
                      </span>
                    </a>
                  </div>
                </div>
              </div>
            </div>
          </div>

          <div
            className="wp-block-niche-group pt-[80px]"
            id="niche-group-68e48824-e5e9-4cfc-86d0-c0bdc448e88e"
          >
            <div className="wp-block-niche-group__inner">
              <div
                className="wp-block-niche-group pt-[40px] pl-[20px] pb-[20px] pr-[20px] md:pl-[40px] md:pr-[40px] niche-blocks--max-width-900 wp-block-niche-group--card"
                id="niche-group-738673a9-3e3c-43a0-8424-e62f579e389a"
              >
                <div className="wp-block-niche-group__inner">
                  <a id="advertise"></a>

                  <div className="is-layout-flex wp-container-28 wp-block-columns">
                    <div
                      className="is-layout-flow wp-block-column"
                      style={{ flexBasis: "20%" }}
                    >
                      <div className="wp-block-image">
                        <figure className="aligncenter size-large is-resized">
                          <img
                            decoding="async"
                            loading="lazy"
                            src="https://www.niche.com/about/wp-content/uploads/2018/04/advertise.png"
                            alt=""
                            className="wp-image-2188"
                            width="125"
                            height="125"
                            srcset="
                              https://www.niche.com/about/wp-content/uploads/2018/04/advertise.png         250w,
                              https://www.niche.com/about/wp-content/uploads/2018/04/advertise-150x150.png 150w
                            "
                            sizes="(max-width: 125px) 100vw, 125px"
                          />
                        </figure>
                      </div>
                    </div>

                    
                  </div>
                </div>
              </div>
            </div>
          </div>

          <div
            className="wp-block-niche-group niche-blocks--max-width-580 pt-[80px] pl-[20px] pr-[20px]"
            id="niche-group-ce9e8d25-d394-4f72-b3ba-54ce7e7858f4"
          >
            <div className="wp-block-niche-group__inner">

              <div
                id="text-block_5edfea445d85a"
                className="Expedite-Text pb-[40px] Expedite-Text__source--700 Expedite-Text__weight--400 niche-blocks--leading-12 niche-blocks--color-gray02 niche-blocks--text-align-left"
              >
                Let's get you started with Dwellics partnership
              </div>

              <div id="sg-form-block_5edfe9795d857" className="niche-blocks-sg">
                <div id="survey-wrapper-4239997" className="sg-survey">
                  <div
                    className="sg-body sg-page-1 sg-pageid-1 sg-webkit sg-ltr sg-replace-icons sg-js-embed"
                    id="sgbody-4239997"
                  >

                   
                    <form>
                        <div className="mb-1">
                            <label for="email" className="text-sm font-medium text-gray-900 dark:text-white">Company Name</label>
                            <input type="text" id="email" className="p-20 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 h-10 p-20" placeholder="john.doe@company.com" required />
                        </div> 
                        <div className="mb-1">
                            <label for="email" className="text-sm font-medium text-gray-900 dark:text-white">Your Name</label>
                            <input type="text" id="email" className="p-20 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 h-10 p-20" placeholder="john.doe@company.com" required />
                        </div> 
                        <div className="mb-1">
                            <label for="email" className="text-sm font-medium text-gray-900 dark:text-white">Your Email</label>
                            <input type="text" id="email" className="p-20 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 h-10 p-20" placeholder="john.doe@company.com" required />
                        </div> 
                        <div className="mb-1">
                            <label for="email" className="text-sm font-medium text-gray-900 dark:text-white">Your Role</label>
                            <input type="text" id="email" className="p-20 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 h-10 p-20" placeholder="john.doe@company.com" required />
                        </div> 
                        <div className="mb-1">
                            <label for="email" className="text-sm font-medium text-gray-900 dark:text-white">What Are You Interested In?</label>
                            <input type="text" id="email" className="p-20 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 h-10 p-20" placeholder="john.doe@company.com" required />
                        </div> 
                        <div className="mb-1">
                            <label for="email" className="text-sm font-medium text-gray-900 dark:text-white">Tell Us More</label>
                            <input type="text" id="email" className="p-20 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 h-10 p-20" placeholder="john.doe@company.com" required />
                        </div> 
                        
                        <button type="submit" className="text-white bg-[#115946] hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Submit</button>
                    </form>
                    {/* <form
                      action="https://www.surveygizmo.com/s3/4239997/About-Pages-Partnership-Contact-Form?__passthrough=true&amp;__output=embedjs"
                      method="post"
                      enctype="multipart/form-data"
                      className="sg-survey-form"
                      id="sg_FormFor4239997"
                    >
                      <div className="sg-hidden-inputs">
                        <input
                          type="hidden"
                          name="sg_referer"
                          value="https://www.niche.com/"
                        />
                        <input
                          type="hidden"
                          name="sg_navchoice"
                          id="sg_navchoice"
                          value=""
                        />
                        <input
                          type="hidden"
                          name="sg_currentpageid"
                          id="sg_currentpageid"
                          value="1"
                        />
                        <input
                          type="hidden"
                          name="sg_surveyident"
                          id="sg_surveyident"
                          value="4239997"
                        />
                        <input
                          type="hidden"
                          name="__ref"
                          id="__ref"
                          value="https://www.niche.com/about/partnerships/"
                        />
                        <input
                          type="hidden"
                          name="sg_interactionlevel"
                          id="sg_interactionlevel"
                          value="0"
                        />
                        <input
                          type="hidden"
                          name="sg_sessionid"
                          id="sg_sessionid"
                          value="1674521872_63cf2d109f24b2.23009676"
                        />
                        <input
                          type="hidden"
                          name="sg_high_contrast"
                          id="sg_high_contrast"
                          value=""
                        />
                        <input
                          type="hidden"
                          name="sg_cint_panelist"
                          id="sg_cint_panelist"
                          value=""
                        />
                      </div>
                      <div className="sg-wrapper">
                        <div className="sg-header">
                          <div className="sg-header-hook-1"></div>
                          <img
                            className="sg-header-image"
                            src="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7"
                            alt=""
                          />

                          <h1 className="sg-title" tabindex="0">
                            <span>
                              About Pages: Partnership Contact Form -
                              [Partnerships Form Fill - K12/College Partnership
                              Request]
                            </span>
                          </h1>

                          <div className="sg-header-hook-2"></div>
                        </div>
                        <div className="sg-content" id="sg-skipnav-target">
                          <div className="sg-content-hook-1"></div>

                          <div className="sg-question-set">
                            <div
                              id="sgE-4239997-1-7-box"
                              className="sg-question sg-type-textbox"
                            >
                              <input
                                type="hidden"
                                id="sgE-4239997-1-7-meta"
                                name="sgE-4239997-1-7-meta"
                                value="hidden=false&amp;required=false"
                              />
                              <input
                                type="hidden"
                                id="sgE-4239997-1-7-time"
                                name="sgE-4239997-1-7-time"
                                value=""
                              />
                              <div className="sg-question-title">
                                <label for="sgE-4239997-1-7-element">
                                  <span className="sg-question-number">1.</span>
                                  School/Company Name
                                </label>
                              </div>
                              <div className="sg-question-options">
                                <div className="sg-control-text sg-control-text">
                                  <input
                                    type="text"
                                    className="sg-input sg-input-text"
                                    id="sgE-4239997-1-7-element"
                                    name="sgE-4239997-1-7"
                                    title="School/Company Name"
                                    value=""
                                  />
                                </div>
                              </div>
                            </div>

                            <div
                              id="sgE-4239997-1-2-box"
                              className="sg-question sg-type-textbox"
                            >
                              <input
                                type="hidden"
                                id="sgE-4239997-1-2-meta"
                                name="sgE-4239997-1-2-meta"
                                value="hidden=false&amp;required=false"
                              />
                              <input
                                type="hidden"
                                id="sgE-4239997-1-2-time"
                                name="sgE-4239997-1-2-time"
                                value=""
                              />
                              <div className="sg-question-title">
                                <label for="sgE-4239997-1-2-element">
                                  <span className="sg-question-number">2.</span>
                                  Your Name
                                </label>
                              </div>
                              <div className="sg-question-options">
                                <div className="sg-control-text sg-control-text">
                                  <input
                                    type="text"
                                    className="sg-input sg-input-text"
                                    id="sgE-4239997-1-2-element"
                                    name="sgE-4239997-1-2"
                                    title="Your Name"
                                    value=""
                                  />
                                </div>
                              </div>
                            </div>

                            <div
                              id="sgE-4239997-1-6-box"
                              className="sg-question sg-type-textbox"
                            >
                              <input
                                type="hidden"
                                id="sgE-4239997-1-6-meta"
                                name="sgE-4239997-1-6-meta"
                                value="hidden=false&amp;required=false"
                              />
                              <input
                                type="hidden"
                                id="sgE-4239997-1-6-time"
                                name="sgE-4239997-1-6-time"
                                value=""
                              />
                              <div className="sg-question-title">
                                <label for="sgE-4239997-1-6-element">
                                  <span className="sg-question-number">3.</span>
                                  Your Email
                                </label>
                              </div>
                              <div className="sg-question-options">
                                <div className="sg-control-text sg-control-text">
                                  <input
                                    type="text"
                                    className="sg-input sg-input-text"
                                    id="sgE-4239997-1-6-element"
                                    name="sgE-4239997-1-6"
                                    title="Your Email"
                                    value=""
                                  />
                                </div>
                              </div>
                            </div>

                            <div
                              id="sgE-4239997-1-10-box"
                              className="sg-question sg-type-textbox"
                            >
                              <input
                                type="hidden"
                                id="sgE-4239997-1-10-meta"
                                name="sgE-4239997-1-10-meta"
                                value="hidden=false&amp;required=false"
                              />
                              <input
                                type="hidden"
                                id="sgE-4239997-1-10-time"
                                name="sgE-4239997-1-10-time"
                                value=""
                              />
                              <div className="sg-question-title">
                                <label for="sgE-4239997-1-10-element">
                                  <span className="sg-question-number">4.</span>
                                  Your Role
                                </label>
                              </div>
                              <div className="sg-question-options">
                                <div className="sg-control-text sg-control-text">
                                  <input
                                    type="text"
                                    className="sg-input sg-input-text"
                                    id="sgE-4239997-1-10-element"
                                    name="sgE-4239997-1-10"
                                    title="Your Role"
                                    value=""
                                  />
                                </div>
                              </div>
                            </div>

                            <div
                              id="sgE-4239997-1-4-box"
                              className="sg-question sg-type-menu"
                            >
                              <input
                                type="hidden"
                                id="sgE-4239997-1-4-meta"
                                name="sgE-4239997-1-4-meta"
                                value="hidden=false&amp;required=false"
                              />
                              <input
                                type="hidden"
                                id="sgE-4239997-1-4-time"
                                name="sgE-4239997-1-4-time"
                                value=""
                              />
                              <div className="sg-question-title">
                                <label for="sgE-4239997-1-4-element">
                                  <span className="sg-question-number">5.</span>
                                  What Are You Interested In?
                                </label>
                              </div>
                              <div className="sg-question-options">
                                <select
                                  id="sgE-4239997-1-4-element"
                                  name="sgE-4239997-1-4"
                                  className="sg-input sg-input-menu"
                                >
                                  <option
                                    label="-- Please Select --"
                                    value="NoAnswer"
                                  >
                                    -- Please Select --
                                  </option>
                                  <option
                                    value="10008"
                                    label="K-12 or College Partnership"
                                  >
                                    K-12 or College Partnership
                                  </option>
                                  <option
                                    value="10009"
                                    label="Purchasing Niche Data"
                                  >
                                    Purchasing Niche Data
                                  </option>
                                  <option
                                    value="10010"
                                    label="Advertising on Niche"
                                  >
                                    Advertising on Niche
                                  </option>
                                  <option value="10011" label="Other">
                                    Other
                                  </option>
                                </select>
                              </div>
                            </div>

                            <div
                              id="sgE-4239997-1-8-box"
                              className="sg-question sg-type-essay"
                            >
                              <input
                                type="hidden"
                                id="sgE-4239997-1-8-meta"
                                name="sgE-4239997-1-8-meta"
                                value="hidden=false&amp;required=false"
                              />
                              <input
                                type="hidden"
                                id="sgE-4239997-1-8-time"
                                name="sgE-4239997-1-8-time"
                                value=""
                              />
                              <div className="sg-question-title">
                                <label for="sgE-4239997-1-8-element">
                                  <span className="sg-question-number">6.</span>
                                  Tell Us More
                                </label>
                              </div>
                              <div className="sg-question-options">
                                <textarea
                                  id="sgE-4239997-1-8-element"
                                  name="sgE-4239997-1-8"
                                  className="sg-input sg-input-essay"
                                  cols="40"
                                  rows="5"
                                ></textarea>
                              </div>
                            </div>
                          </div>

                          <div className="sg-content-hook-2"></div>

                          <div className="sg-footer">
                            <div className="sg-footer-hook-1"></div>

                            <div className="sg-button-bar">
                              <input
                                type="submit"
                                className="sg-button sg-submit-button"
                                id="sg_SubmitButton"
                                name="sGizmoSubmitButton"
                                aria-label="submit button"
                                onclick="$SG('sg_navchoice').value='sGizmoSubmitButton';return(true);"
                                value="Submit"
                              />
                            </div>
                            <div
                              className="sg-progress-bar"
                              role="progressbar"
                              aria-valuemin="0"
                              aria-valuemax="100"
                              aria-valuenow="0"
                            >
                              <div className="sg-progress-bar-text">0%</div>
                              <div className="sg-progress-bar-background">
                                <div
                                  className="sg-progress-bar-inner"
                                  style={{ width: "0%" }}
                                ></div>
                              </div>
                            </div>

                            <div className="sg-footer-hook-2"></div>
                          </div>
                        </div>
                      </div>
                    </form> */}
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}

export default partner;
